...
export const BEGINNING_CASH_BALANCE_REGEX_PATTERN = /Beginning Cash Balance as of \d{2}\/\d{2}\/\d{4}-?\d+\.\d{2}/gi;
...
export const TRANSACTION_TYPE_REFERENCE_REGEX_PATTERN =
  /(Check\d+|Bill Pay\d+|Bill Pay Check \d+|eCheck receipt [A-Z]\d+-[A-Z]\d+|eCheck receipt [A-Z\d]+-[A-Z\d+]{4}|eCheck\s?\s?[A-Z\d]+-[A-Z\d+]{4}|[A-Za-z0-9]{4}-[A-Za-z0-9]{4}|\s?CC |Receipt\s?\d+-\d+|Receipt\s?\d+|Receipt)/gi;
